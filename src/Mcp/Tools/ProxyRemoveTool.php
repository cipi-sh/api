...
#[Description('Remove a prefix reverse proxy from an app (cipi proxy remove). Requires Cipi CLI ≥ 5.4.1.')]
class ProxyRemoveTool extends Tool
{
    ...
    public function handle(Request $request): Response
    {
        ...
        try {
            return Response::text(json_encode($this->routes->removeProxy($name, $prefix), JSON_PRETTY_PRINT));
    ...
    public function schema(JsonSchema $schema): array
    {
        return [
            ...
            'prefix' => $schema->string()->description('Prefix of the proxy to remove (e.g. /api/)')->required(),
        ];
    }
}
